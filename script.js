...
// Mobile optimization: phones struggle with video background + mp3 decoding for beat analysis.
// Keep the same look, but reduce heavy work on small/touch devices.
const IS_MOBILE_LITE = window.matchMedia('(max-width: 768px), (pointer: coarse)').matches;
const MOBILE_MAX_BARS = 28;
const VISUALIZER_MIN_INTERVAL = IS_MOBILE_LITE ? 72 : 0;
...
function tryPlayVideo() {
  ...
  if (IS_MOBILE_LITE) {
    // On mobile, the background video is the biggest cause of lag.
    // Disable it and let the lightweight gradient fallback take over.
    bgVideo.pause();
    bgVideo.removeAttribute('src');
    bgVideo.querySelectorAll('source').forEach((source) => source.removeAttribute('src'));
    bgVideo.load();
...
async function initBeatData() {
  ...
  if (IS_MOBILE_LITE) {
    // Do not fetch/decode the whole MP3 on phones; it can freeze or lag the page.
    startFallbackVisualizer();
    return;
  }
...
function startBeatVisualizer() {
  ...
  const render = () => {
    ...
    if (VISUALIZER_MIN_INTERVAL && nowMs - lastVisualizerPaint < VISUALIZER_MIN_INTERVAL) return;
    lastVisualizerPaint = nowMs;

    const bars = Array.from(spectrumBars).slice(0, IS_MOBILE_LITE ? MOBILE_MAX_BARS : spectrumBars.length);
...
function setupEvents() {
  if (IS_MOBILE_LITE) {
    document.body.classList.add('mobile-lite-mode');
    spectrumBars.forEach((bar, index) => {
      if (index >= MOBILE_MAX_BARS) bar.style.display = 'none';
    });
  }
  if (enterBtn) {
